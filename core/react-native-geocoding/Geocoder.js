...
export default Geocoder = {
  ...
  async from(...params) {
    ...
    // {latitude, longitude}  or {lat, lng}
      ...
      if (params[0].place_id) {
        queryParams = {
          place_id: params[0].place_id,
        };
      } else {
        queryParams = {
  ...
  async autoSuggestList(...params) {
    ...
    // address
    if (typeof params[0] === 'string') queryParams = { input: params[0] };
    ...
    queryParams = { key: this.apiKey, ...this.options, ...queryParams };
    // build url
    const url = `https://maps.googleapis.com/maps/api/place/autocomplete/json?${toQueryParams(
      queryParams
    )}`;
