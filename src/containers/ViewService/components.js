...
import { ProductUtil, ServicesUtil } from '../../DataUtils';
import { strings } from '../../utils/i18n';
import { AppStyles, Metrics } from '../../theme';
import { Text } from '../../components';
import styles from './styles';
import { getServicesItem } from '../../ducks/services/selectors';
import { API_SERVICE_REVIEWS } from '../../config/WebService';

const TitlePriceRating = React.memo(({ data, price }) => (
  <>
    ...
    <StarRating
      ...
      style={styles.rating}
      count={ProductUtil.getAverageRating(data)}
...
const RatingList = React.memo(({ _id }) => {
  const data = useSelector(getServicesItem(_id));
  const ratingCount = ServicesUtil.ratingCount(data);
  const averageRating = ServicesUtil.rating(data);
  const ratingData = ServicesUtil.getRecentRating(data);
  const isRated = ratingData?.length > 0;
  const url = API_SERVICE_REVIEWS;
  const payload = { service_id: _id };
