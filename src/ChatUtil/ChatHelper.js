...
function checkRoomExists(roomName) {
  ...
  if (rooms[roomName] && rooms[roomName]._id) {
...
async function onMessageReceive(err, message, messageOptions) {
  ...
  if (!err) {
    ...
    // if (message.u.username === getRocketChatUserName()) return;
...
async function disconnectSocket(isLogout = false) {
  if (isSocketConnected || isConnecting) {
    try {
      ...
      if (isLogout) {
        SDK.driver.ddp.removeListener('disconnected');
        SDK.driver.ddp.removeListener('connected');
        ddp = null;
...
async function loginToRocketChat() {
  try {
    ...
    const { data } = DataHandler.getStore().getState().auth;
    const chatAccessToken = data?.rocket_chat_account?.data?.authToken ?? '';

    // set is connecting true
    isConnecting = true;
    DataHandler.getStore().dispatch(
    ...
    // check if need to connect to room after connect
    checkRoomJoinAfterConnect();
  } catch {
    disconnectSocket();
    checkRoomJoinAfterDisConnect();
  }
}

function checkRoomJoinAfterConnect() {
...
function checkRoomJoinAfterDisConnect() {
  if (Util.isNotEmpty(roomJoinData)) {
...
function addListenersToDdp() {
  // events connected and disconnected
  SDK.driver.on('disconnected', onDisconnected);
  SDK.driver.on('connected', onConnected);

  // response to message when change in stream
  SDK.driver.reactToMessages(onMessageReceive);
}

async function connectToDdp() {
  // set base url api
...
function onInternetConnect() {
  if (ddp) {
    //console.log('ddddddp is connected');
  } else {
    //console.log('ddddddp is null');
    connectToSocket();
  }
}
...
async function connectToSocket(isFirstTime = false) {
  ...
  // if it is first time set socket status to connecting
  if (isFirstTime) {
  ...
  connectSocketTimeOut = setTimeout(async () => {
    if (
      (isConnecting === false || isFirstTime) &&
      isSocketConnected === false
    ) {
      // get chat user token and rocketchat user name
      const { data } = DataHandler.getStore().getState().auth;
      const chatAccessToken = data?.rocket_chat_account?.data?.authToken ?? '';
      const rocketChatUserName =
        data?.rocket_chat_account?.data?.me?.username ?? '';

      // have rocket chat object
      if (rocketChatUserName !== '' && chatAccessToken !== '') {
        if (ddp) {
          // login to rocket
          loginToRocketChat();
        } else {
